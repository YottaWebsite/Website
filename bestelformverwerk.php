...
if (isset($_POST["submitknop"])) {
    if (empty($_POST["domeinnaam"]) ||
            ...
            empty($_POST["email"]) ||
            empty($_POST["ext"]) ||
            empty($_POST["Domeinoptie"]) ||
            empty($_POST["hosting"])) {
        print ("Alle velden zijn verplicht");
    } ELSE {
        $link = mysqli_connect("test.local", "root", "usbw", "Bestellingen", 3307);
        if (isset($_POST['submitknop'])) {
        ...
        print ("Uw bestelling is succesvol geplaats! <br>");
    }
}
?>
                    <a href="bestelform.php">Terug naar de domeinregistratie
